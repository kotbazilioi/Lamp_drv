...
#define configUSE_PREEMPTION 1 //Выбор режима ядра(1-вытесняющий, 0-кооперативный) 
#define configUSE_IDLE_HOOK 1 //Задача "Бездействие"(idle hook) (1-используется, 0-не используется)  
#define configUSE_TICK_HOOK 0 //Функция перехвата ТИКА (1-используется, 0-не используется)
...
#define configTOTAL_HEAP_SIZE ((size_t)(32 * 1024))// */ /* not used by heap_3 allocator *///Общий объем памяти (кучи), только для heap_1, heap_2 
...
#define configIDLE_SHOULD_YIELD 1 //Определяет поведение задач с приоритетом idle hook 
...
#define INCLUDE_vTaskPrioritySet                1 //включают/отключают использование API-функций (в данном случае vTaskPrioritySet)
#define INCLUDE_uxTaskPriorityGet               1
#define INCLUDE_vTaskDelete                     1
#define INCLUDE_vTaskSuspend                    1
...
#define portGET_RUN_TIME_COUNTER_VALUE()   1      // ulMainGetRunTimeCounterValue()
...
#define configKERNEL_INTERRUPT_PRIORITY (configLIBRARY_LOWEST_INTERRUPT_PRIORITY << (8 - configPRIO_BITS)) //- Используется только процессорами Cortex-M3
